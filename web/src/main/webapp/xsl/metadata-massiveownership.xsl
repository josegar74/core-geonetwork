...
<!-- 
     - uses xml/metadata-massiveNewOwner.xml on /root/gui/info
     - is the output sheet for the service metadata.PrepareMassiveNewOwner
	...
	<xsl:template name="content">
		<xsl:call-template name="formLayout">
			...
			<xsl:with-param name="buttons">
				<div id="massiveNewOwnerButtons" style="float:left;width:100%">
					<center>
      			<button class="content" onclick="checkMassiveNewOwner('metadata.massive.newowner','{concat(/root/gui/strings/results,' ',/root/gui/strings/massiveNewOwnerTitle)}')" type="button">
